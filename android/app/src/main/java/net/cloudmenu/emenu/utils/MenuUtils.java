...
public class MenuUtils {
    ...
    public static List<GoodsCategory> getMenuGoodsCategories(Menu menu) {
        ...
        for (int i = 0, menuLen = menu.getPagesSize(); i < menuLen; i++) {
            ...
            if (lastCategory == null
                ...
                if (lastCategory != null) {
                    lastCategory.end = totalDishCount;
                }
                lastCategory = new GoodsCategory(goods.getCategory());
                lastCategory.start = totalDishCount + 1;
                set.add(lastCategory);
            }
            totalDishCount += page.getGoodsListSize();
        }
        if (lastCategory != null)
            lastCategory.end = totalDishCount;
        return set;
    ...
    public static String getGoodPreviewImgUrl(Context context, Goods good) {
        if (good.getImgs() != null && good.getImgs().size() > 0) {
            String url = good.getImgs().get(0).getPreviewImgUrl();

            return MenuUtils.getImgUrl(context, url);
        }
        return null;
    }

    public static String getGoodImgUrl(Context context, Goods good) {
        if (good.getImgs() != null && good.getImgs().size() > 0) {
            String url = good.getImgs().get(0).getImgUrl();
            return MenuUtils.getImgUrl(context, url);
        }
        return null;
    }
